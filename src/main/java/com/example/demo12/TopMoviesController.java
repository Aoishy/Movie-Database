...
public class TopMoviesController {
    ...
    public void ClickToGetTopMovies(ActionEvent actionEvent)throws IOException {
        long profit1;
        long profit2;
        List<Movie>toplist= new ArrayList<>();
        for (int i = 0; i <Client.list.size();i++)
        {
            for(int k=i+1;k<Client.list.size()-1;k++)
            {
                profit1=Client.list.get(i).Revenue-Client.list.get(i).Budget;
                profit2=Client.list.get(k).Revenue-Client.list.get(k).Budget;
                if(profit1<profit2)
                {
                    Collections.swap(Client.list,i,k);
                }
            }
        }
        for(int j=0;j<5;j++)
        {
            toplist.add(Client.list.get(j));
